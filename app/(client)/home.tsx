...
const TABS: Tab[] = ["proximos", "historico", "cancelados"];
const TAB_PADDING = 4;
const TAB_GAP = 4;
...
export default function ClientHomeScreen() {
  ...
  const segmentWidth = tabsWidth > 0 ? (tabsWidth - TAB_PADDING * 2 - TAB_GAP * 2) / 3 : 0;
  const indicatorX = useSharedValue(0);

  useEffect(() => {
    if (segmentWidth > 0) {
      indicatorX.value = withSpring(TAB_PADDING + activeIndex * (segmentWidth + TAB_GAP), {
        damping: 20,
        stiffness: 220,
      });
    }
    ...
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.tabs} onLayout={(e) => setTabsWidth(e.nativeEvent.layout.width)}>
          {segmentWidth > 0 && <Animated.View style={[styles.tabIndicator, indicatorStyle]} />}
          {TABS.map((t) => (
          ...
          {showList && (
            <View style={{ gap: space.m }}>
              {visibleOrders.map((order, index) => {
                ...
                return (
                  <Animated.View key={order.order_id} entering={FadeInDown.delay(index * 40).duration(280)}>
                    <Pressable style={styles.orderCard} onPress={() => router.push(`/(client)/pedido/${order.order_id}`)}>
...
const styles = StyleSheet.create({
  ...
  tab: { flex: 1, paddingVertical: 10, alignItems: "center", borderRadius: radius.m, zIndex: 1 },
  tabIndicator: {
    position: "absolute",
    top: TAB_PADDING,
    bottom: TAB_PADDING,
